...
function populateFacilitation(){const saved=facilitationContent[facilitationKey()]||{};["heading","body","scripture","questions","takeaway"].forEach(name=>facilitationForm.elements[name].value=saved[name]||"");facilitationPreview.href=`facilitation-item.html?stage=${facilitationStage.value}&item=${facilitationItem.value}`;}
...
facilitationForm.addEventListener("submit",event=>{event.preventDefault();facilitationContent[facilitationKey()]={heading:facilitationForm.elements.heading.value.trim(),body:facilitationForm.elements.body.value.trim(),scripture:facilitationForm.elements.scripture.value.trim(),questions:facilitationForm.elements.questions.value.trim(),takeaway:facilitationForm.elements.takeaway.value.trim()};localStorage.setItem(FACILITATION_ADMIN_KEY,JSON.stringify(facilitationContent));showToast("User page created");});
