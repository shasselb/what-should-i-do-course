...
function sortInto(pile){
  ...
  setTimeout(()=>{state.queue.shift();state.piles[pile].push(card.id);state.history.push({id:card.id,pile});if(pile==="known")state.confirmedAt="";state.revealed=false;activeCard.classList.remove(animation);render();},190);
}
function undo(){const move=state.history.pop();if(!move)return;state.piles[move.pile]=state.piles[move.pile].filter(id=>id!==move.id);if(move.type==="reclassify"){state.piles[move.source].push(move.id);}else{state.queue.unshift(move.id);}if(move.pile==="known"||move.source==="known")state.confirmedAt="";state.revealed=false;showToast("Last move undone");render();}
